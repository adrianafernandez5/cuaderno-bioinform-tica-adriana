...
Para ello: https://www.ebi.ac.uk/interpro/api/protein
https://www.ebi.ac.uk/interpro/api/protein/taxonomy

Ahora definimos un objeto para todas las proteínas de UniProt y otro
solo para las que tienen asignadas un taxón

```{r}
Taxonomia <- fromJSON(paste0(API, 'taxonomy'))
str(Taxonomia)
todos <- Taxonomia[['taxa']][['uniprot']]
todos
```

```{r}
# Taxones que tienen proteínas asignadas
Tpro <- fromJSON(paste0(API, 'taxonomy/uniprot/protein'))
str(Tpro)
Taxconprot <- Tpro[['count']]
Taxconprot
```
...
En total, hay 1.387.422 taxones que no tienen ninguna proteína asignada.

Para la estructura terciaria, creamos un objeto con la información de
estructuras y luego accedemos a los datos.

```{r}
todas <- fromJSON(paste0(API, 'protein'))
str(todas)
Proteinas <- todas[['proteins']][['uniprot']]
Proteinas
```

```{r}
estructura <- fromJSON(paste0(API, 'protein/structure'))
str(estructura)
ProtconEstructura3a <- estructura[['proteins']][['uniprot']]
ProtconEstructura3a
```
...
Se conocen 15 proteínas de cocodrilos con el dominio *zinc finger, C2HC
type* (Pfam PF01530).
