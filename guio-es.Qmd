...
# para copiar un archivo de una carpeta a otro, hacemos una copia con el comando cp (copy) o mv (move). primero hay que decirle el origen. hayq eu usar el tabulador (la flecha a al izquierda) para no equivocarse. esto es para usar el directorio raíz
#otra opción es usar un camino más dorecto. sería cp../../../.. cada dos puntos es un nivel anterior de carpeta, lo vas subiendo. esto se llama path relativo, dependiendo de dodne estas iras a un sitio u otro. 
::: {.callout-tip title="Consejo"}
Amplía la tabla modificando el documento fuente.
:::
# en R hay un paquete que se lama DECIPHER muy bueno, no es tan importante entender EMBOSS, que es más viejo que la picor. vamos a intentar instalar emboss, yo gracias a chat gpt ya lo tengo.
...
```
:::
# LO QUE TENDRIAMOS QUE HABER HECHO Y NO HEMOS HECHO. toda la práctica vaya
#wget es para descargar de google un pa de secuencias y needle es para alinearlas. para que no tenga que ejecutarlo todo todo el rato y todas la sveces que se abre el script se hace una ejecucioñon condicional. if [! -e aln1.txt ]; then (saltamos de linea) wget -o nombre de lo que queremos descargar o su dirección. las barras invertidas(\) nos permite distribuir los argumentos de una linea. lo que hace es ignorar lo que viene siguiente.
# needle implementa un algoritmo de alineación de secuencia. te da un alineamiento del cromosoma en formato .txt. los alineamientos no siempre dan bien, pero siempre dan un resultado.
#exonerate es mñas inteligente, permite hacer otras cosas. se puede ir diciendo lo que stas haceindo. si lo que estamos alienado es un mRNA procesado si intrones, y se alinea a un cromosoma. el alineamiento es muy pobre y hay   ue decirle que no busque los intrones. que busque fragmentos de mrna y lo alinee con fragmentos del cromosoma. àra que el alineamiento salga en pantalla hay que no decirle nada para que te lo ponga en el mismo documento que estas usando. si usas exonerate... > archivo...  > es para redirigir .
# esto era para un alineamiento de dos secuendias. si queremos alienar mas hay que usar otro algortimo, como el muscle 
